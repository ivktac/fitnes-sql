...
CREATE VIEW groups_coach AS
  SELECT sp.name, g.coach_passport, c.sport_kind
  FROM groups g
...
CREATE VIEW yoga_coaches AS
  ...
  WHERE s.datetime BETWEEN now() AND now() + '1 month'
    AND c.sport_kind = (SELECT sport_kind_id 
                        FROM sport_kind
                        WHERE name = 'Yoga');

SELECT *  FROM yoga_coaches;
...
CREATE VIEW coach_taxes AS
  ...
  FROM subscription s
    ...
    JOIN coach c ON g.coach_passport = c.passport
  WHERE p.date BETWEEN now() - INTERVAL '1 month' AND now()
  AND tax > 0
    GROUP BY c.passport;

SELECT * FROM coach_taxes;
...
CREATE VIEW gain_sport_kind AS 
  ...
  FROM coach c
    JOIN groups g 
    ON c.passport = g.coach_passport
      JOIN schedule s 
      ON g.group_id = s.group_id
        JOIN sport_kind sk 
        ON c.sport_kind = sk.sport_kind_id
    GROUP BY sk.name;

SELECT * FROM gain_sport_kind;

-- 4. К-сть усіх проведених занять та ранжування кожного тренера за кількістю занять

SELECT c.passport, c.full_name, gc.cnt do_count,
  RANK() OVER (ORDER BY gc.cnt DESC) do_rank
FROM coach c
JOIN (SELECT coach_passport, COUNT(*) AS cnt
      FROM groups
      GROUP BY coach_passport) gc
ON gc.coach_passport = c.passport;
...
SELECT * FROM groups_with_more_10_visitors;
...
CREATE VIEW schedule_year AS
  ...
  FROM schedule s
  WHERE s.datetime BETWEEN now() AND now() + INTERVAL '1 year'
    AND group_id = 2
  GROUP BY group_id, datetime
...
SELECT 'Visitors' as group_type, group_id
FROM group_visitors
  GROUP BY(group_id)
  HAVING COUNT(*) > 2
UNION ALL
SELECT 'Coaches' as group_type, group_id
FROM groups
  GROUP BY coach_passport, group_id
  HAVING COUNT(*) > 2
...
FROM coach c
  JOIN groups g ON c.passport = g.coach_passport
WHERE sport_kind = (SELECT sport_kind_id
                    FROM sport_kind
                    WHERE name = 'Yoga');
...
CREATE VIEW unpaid_visitors AS
  ...
  WHERE p.date() <= now()
    AND p.subscription_id IN (SELECT subscription_id 
                              FROM subscription
                              WHERE name = 'once');

SELECT * FROM unpaid_visitors;
...
SELECT passport, full_name, sport_kind
FROM sport_coach sc1
WHERE salary IN (SELECT MAX(sc2.salary)
                FROM sport_coach sc2
                WHERE sc2.sport_kind = sc1.sport_kind
                  GROUP BY sc2.sport_kind)
  ORDER BY sport_kind, full_name;
